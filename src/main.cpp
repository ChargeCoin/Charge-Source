...
#if defined(NDEBUG)
# error "Charge cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc38cd9e629914cb8bde2a50dc81eed31573fb376f081d026acc59dc91c4ee72a");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Charge: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Charge Signed Message:\n";
...
{
    // Charge: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    int64 nSubsidy = 25 * COIN;
            ...
            if(nHeight == 1)
            {
            nSubsidy = 146465 * COIN;
            }

    // Subsidy is cut in half every 290000 blocks
    nSubsidy >>= (nHeight / 290000);
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Charge: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Charge: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        // Genesis Block:
        // CBlock(hash=12a765e31ffd4059bada, PoW=0000050c34a64b415b6b, ver=1, hashPrevBlock=00000000000000000000, hashMerkleRoot=97ddfbbae6, nTime=1518210623, nBits=1e0ffff0, nNonce=0, vtx=1)
        ...
        // Genesis block
        const char* pszTimestamp = "BBC News Feb 9 2018 US markets sag as volatility continues";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 25 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("04274fd0af59d78b247aac86f40d2090470e48bc0104da10117a3c4e09c7cb7cec152f70fb51238bb5f77f7748e4eacb9a3d7fc70c64f745a475b2c7ec331ed902") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1518210623;
        ...
        assert(block.hashMerkleRoot == uint256("0xa4b66e67275f17976d8e10ef7c455c9f4302bdec7a9cd55fbac470e57d6b4f10"));
...
unsigned char pchMessageStart[4] = { 0x81, 0xf8, 0xf0, 0xbe }; // Charge: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ChargeMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ChargeMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ChargeMiner(CWallet *pwallet)
{
    printf("ChargeMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("charge-miner");
    ...
    try { loop {
        ...
        printf("Running ChargeMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ChargeMiner, pwallet));
}
